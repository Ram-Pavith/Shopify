...
const offerApplyCartDb = async ({cart_id,user_id})=>{
    ...
    const {rowCount:insertRowCount} = await pool.query(
        ...
        SELECT ci.cart_item_id,po.offer_id,o.name,o.discount_value from cart_item as ci join product_offers as po on ci.product_id = po.product_id join offers as o on o.offer_id = po.offer_id where ci.cart_id=$1;
        `,
        [cart_id]
    );
    ...
    const {rows:groupedOffer} = await pool.query(`select cioa.offer_id from cart_item_offers_applied as cioa join cart_item as ci on ci.cart_item_id = cioa.cart_item_id join offers as o on o.offer_id = cioa.offer_id where o.offer_type='GROUPED_PRODUCT' and ci.cart_id = $1`,[cart_id])
    console.log("grouped Offer ",groupedOffer)
    if(groupedOffer[0]!==undefined){
        ...
        const {rows:cartGroupedOfferItems} = await pool.query(`select ci.product_id from cart_item as ci join cart_item_offers_applied as cioa on cioa.cart_item_id = ci.cart_item_id where cioa.offer_id = $1`,[groupedOffer[0].offer_id])
    ...
    const {rowCount:Updatecart} = await pool.query(`update cart set price=$1,total=$2 where cart_id=$3`,[price,(price*1.18)+10.00,cart_id])
